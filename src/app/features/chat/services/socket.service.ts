...
export class SocketService implements OnDestroy {
  ...
  // Presence (online/offline) per channel
  ...
  readonly presence$: Observable<{ channelId: string; onlineUserIds: string[] }> =
  ...
  connect(): void {
    ...
    // Presence updates
    this.socket.on('presence_update', value => this.presenceSubject.next(value));

    this.socket.on('error', error =>
      this.errorsSubject.next(error?.message || 'Realtime chat error')
    );
  }
  ...
  send(channelId: string, payload: { type: string; text: string; attachments?: unknown[] }): boolean {
    if (!this.socket?.connected) return false;
    this.socket.emit('send_message', { channelId, payload });
    return true;
  }

  typing(channelId: string, userName: string, active: boolean): void {
    this.socket?.emit(active ? 'typing_start' : 'typing_stop', { channelId, userName });
  }
