...
export class ChatShellComponent {
  ...
  readonly courses=signal<ChatCourse[]>([]); readonly courseId=signal(''); readonly channelId=signal(''); readonly channels=signal<Channel[]>([]); readonly messages=signal<ChatMessage[]>([]); readonly members=signal<CourseMemberView[]>([]); readonly pinned=signal<ChatMessage[]>([]); readonly files=signal<any[]>([]); readonly progress=signal(0); readonly loading=signal(true); readonly error=signal(''); readonly currentUserId=this.auth.currentUser?.id || ''; readonly canAddCourse=this.auth.currentUser?.role==='instructor';
  ...
  load():void {
    const courseId=this.courseId(); if(!courseId || courseId==='my-courses'){this.loading.set(false);return}
    this.loading.set(true); this.error.set('');
    this.channelsApi.list(courseId).pipe(switchMap(result=>{ const channels=result.data || []; this.channels.set(channels); const selected=this.channelId() || channels[0]?._id || channels[0]?.id || ''; if(selected && selected!==this.channelId()){this.channelId.set(selected); void this.router.navigateByUrl(`${this.router.url.split('/channel/')[0]}/channel/${selected}`);} const progress$=this.auth.currentUser?.role==='student'?this.membersApi.progress(courseId):of({success:true,data:{completedPercent:0}}); return selected ? forkJoin({messages:this.chats.messages(selected),pinned:this.chats.pinned(selected),files:this.chats.files(selected),members:this.membersApi.list(courseId),progress:progress$}) : of(null); }),takeUntilDestroyed(this.destroyRef)).subscribe({next:data=>{if(data){this.messages.set(data.messages.messages || []);this.pinned.set(data.pinned.messages || []);this.files.set(data.files.files || []);this.members.set(data.members.data || []);this.progress.set(data.progress.data?.completedPercent || 0);this.socket.join(this.channelId())}this.loading.set(false)},error:err=>{this.error.set(err.error?.message || 'Unable to load chat');this.loading.set(false)}});
  }
